...
const ShuffleButton = (props: Props) => {
	...
	const fetchFromGit = React.useCallback(async () => {
		...
		promises.push(
			d3.csv<ShuffleData & string>(
				`https://raw.githubusercontent.com/wevisdemo/thailand-election-2023/main/apps/mpasset/crawler/public/data/shuffle-index.csv`,
				d3.autoType
			)
		);
		promises.push(
			d3.csv<ShuffleData & string>(
				`https://raw.githubusercontent.com/wevisdemo/thailand-election-2023/main/apps/mpasset/crawler/public/data/yourcandidate/shuffle-index.csv`,
				d3.autoType
			)
		);
	...
	const handleClick = () => {
		...
		const pIndex =
			selectedDataSet === 'นักการเมือง 62'
				? shuffleList62[index].index
				: shuffleList66[index].index;
		const personData =
			selectedDataSet === 'นักการเมือง 62'
				? person[pIndex]
				: yourCandidatePerson[pIndex];
		...
		if (p)
			setSelectedPerson({
				...
				Party: {
					...
					Name: p.Name,
					Color: p.Color,
					Images: Array.isArray(p.Images) ? p.Images[0]!.url : null,
				},
			});
		else setSelectedPerson(personData);
