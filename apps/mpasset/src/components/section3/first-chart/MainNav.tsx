...
const MainNav = ({ width, height, filteredPerson }: Props) => {
  ...
  const handleClick = React.useCallback((d: PersonCustom) => {
    ...
    if (p)
      setSelectedPerson({ ...d, Party: { Id: p.Id, Name: p.Name, Color: p.Color, Images: Array.isArray(p.Images) ? `/mpasset/party/${p.Name}.webp` : null } })
    else
      setSelectedPerson(d)
    if (currentStep === 0)
      setCurrentStep(1)
  ...
  React.useMemo(() => {
    if (filteredPerson.length > 0) {
      const data = filteredPerson.filter(d => d.countCompShare > 0 || d.countDirector > 0)
      ...
      const yScaleBand = d3.scaleBand().domain(data.map((_, i) => `${i}`)).range([0, 40 * countPerson]).paddingInner(.5).paddingOuter(.2)
      ...
      const rLogo = 10
      node.filter((d) => typeof d.PartyName === 'string')
        .append('rect')
        .attr('x', (d) => d.totalPctShare! < limiter ?
          d.totalPctShare! < 0 ?
      ...
      node.append('text')
        ...
        .attr('class', 'typo-b7')
        .text((d) => d.totalValueShare !== 0 ? convertToInternationalCurrencySystem(d.totalValueShare!) : (d.countDirector > 0 || d.countCompShare > 0 ? `เกี่ยวข้องกับธุรกิจแต่ไม่มีหุ้นอยู่` : 'ไม่เกี่ยวข้องกับธุรกิจ'))
      ...
      defsEnter.append("svg:image")
        ...
        .attr("xlink:href", (d) => d.Images ? String(`/mpasset/party/${d.Name}.webp`) : '')
        .attr("width", avatar_size)
        .attr("height", avatar_size)
        .attr("x", 0)
        .attr("y", 0);
