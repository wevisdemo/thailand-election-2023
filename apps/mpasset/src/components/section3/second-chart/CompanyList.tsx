		...
		<div>
			{Array.isArray(companyData) && companyData.length > 0 && (
				<div className="pt-[15px] pb-[7px] flex flex-col gap-y-[5px]">
					{type === 'shareholder' ? (
						<>
							<div className="flex flex-row justify-between typo-b5 typo-ibmplex items-center">
								...
								<div className="flex flex-row gap-x-[5px] ">
									<div className="font-bold">
										{Number(selectedPerson?.totalValueShare).toLocaleString(
											'en'
										)}
									</div>
									<div>บาท</div>
							...
							<div className="flex flex-row justify-between typo-b5 typo-ibmplex items-center">
								...
								<div className="flex flex-row gap-x-[5px] ">
									<div className="font-bold">
										{Number(selectedPerson?.countCompShare).toLocaleString(
											'en'
										)}
									</div>
									<div>บริษัท</div>
									{!isExpandShareholder ? (
										<Expand
											...
											onClick={() => setIsExpandShareholder(true)}
										/>
									) : (
										<Collapse
											className="cursor-pointer"
											onClick={() => setIsExpandShareholder(false)}
										/>
									)}
								</div>
							</div>
						...
						<div className="flex flex-row justify-between typo-b5 typo-ibmplex items-center">
							...
							<div className="flex flex-row gap-x-[5px] ">
								<div className="font-bold">
									{Number(companyData.length).toLocaleString('en')}
								</div>
								<div>บริษัท</div>
								{!isExpandShareholder ? (
									<Expand
						...
						<div className="flex flex-row gap-x-[5px] ">
							<div className="font-bold">
								{Array.isArray(companyData)
									? companyData.reduce(
											(acc, cur) =>
												acc + (Array.isArray(cur.gov_fund_proj) ? 1 : 0),
											0
									  )
									: 0}
							</div>
					...
					{isExpandShareholder && (
						<div className={type === 'director' ? 'pb-[150px]' : 'pb-[30px]'}>
							<div className="flex flex-col gap-y-[10px]">
								{companyData
									.sort((a, z) => z.pct_share - a.pct_share)
									.map((data) => (
										<div
											key={`comp-data-${data.company_id}`}
											onClick={() => setSelectedCompany(data)}
											className={`px-[15px] py-[10px] cursor-pointer group flex flex-row rounded-[5px] justify-between items-center ${
												Array.isArray(data.gov_fund_proj)
													? 'bg-black text-white border-[3px] border-transparent'
													: 'text-black bg-white border-[3px] border-black'
											}`}
										>
											<div className="truncate text-ellipsis typo-b5 typo-ibmplex leading-[150%]">
												บริษัท {data.company_name_th}
											</div>
											<div className="flex flex-row gap-x-[12px] items-center flex-nowrap justify-end text-end">
												{type === 'shareholder' && (
													<div className="typo-b4 typo-ibmplex whitespace-nowrap">
														{data.pct_share} %
													</div>
												)}
												<svg
													className="cursor-pointer group-hover::translate-x-2 transition-all"
													width={20}
													height={18}
													viewBox="0 0 20 18"
													fill="none"
