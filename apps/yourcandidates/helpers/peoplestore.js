...
export const getPeople = async (province, zone) => {
  ...
  peoples = peoples.filter((people) => people.Province == province && people.Zone == zone)
  ...
  peoples = peoples.map((people) => 
    ...
    Object.assign(people, peoplesHistory.find((history) => history.Name === people.Name))
    :people
  )
  // join candidates and party (for use image party)
  for(let i=0; i < peoples.length; i++){
    let partyName = await peoples[i].Party
    peoples[i]['Party'] = await parties.find((party) => party.Name === partyName)
  }
