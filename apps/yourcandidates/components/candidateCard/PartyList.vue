  ...
  >
    ...
    >
      ...
      <table class="w-full">
        ...
        <tr
          v-for="(item, index) in sortedPartyList"
          :key="index"
          class="party-list-box"
          :class="item.PastMP ? 'bg-[color:var(--color-highlight-1)]' : ''"
          @click="clickItem(item)"
        >
          <td style="padding: 5px 0px 5px 5px">{{ item.Number }}</td>
...
export default {
  ...
  computed: {
    sortedPartyList() {
      return this.partyList.sort((a, b) => a.Number - b.Number)
    },
  },
}
</script>
