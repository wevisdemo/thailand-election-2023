...
>
	<div>
		...
		<h6 class="typo-h6 font-bold">
			คุณคิดว่า เมื่อเทียบกับการเลือกตั้งปี 62 ความนิยมต่อ ส.ส.
			ของคนไทยเบนไปจากเดิมหรือไม่? ไปทางฝ่ายใด?
		</h6>
		<span
	...
	<div class="w-full flex flex-col items-center">
		<div class="w-full max-w-[400px]">
			<div class="w-full flex justify-between">
				<p class="typo-h7 text-xl">เลือก ส.ส.<br />ฝ่ายค้านมากขึ้น</p>
				<p class="typo-h7 text-xl text-right">เลือก ส.ส.<br />รัฐบาลมากขึ้น</p>
			</div>
			<div class="w-full flex items-center justify-between mt-1">
				<div
					...
					ScaleAns.Scale0
						? 'bg-byx-red'
						: ''}"
					on:click={() => input.updateQuize5(ScaleAns.Scale0)}
				/>
				<div
					...
					ScaleAns.Scale10
						? 'bg-byx-red'
						: ''}"
					on:click={() => input.updateQuize5(ScaleAns.Scale10)}
				/>
				<div
					...
					ScaleAns.Scale30
						? 'bg-byx-red'
						: ''}"
					on:click={() => input.updateQuize5(ScaleAns.Scale30)}
				/>
				<div
					...
					ScaleAns.Scale50
						? 'bg-byx-red'
						: ''}"
					on:click={() => input.updateQuize5(ScaleAns.Scale50)}
				/>
				<div
					class="border border-black w-8 h-8 rounded-full cursor-pointer {ans ==
					ScaleAns.Scale70
						? 'bg-byx-red'
						: ''}"
					on:click={() => input.updateQuize5(ScaleAns.Scale70)}
				/>
				<div
					class="border border-black w-9 h-9 rounded-full cursor-pointer {ans ==
					ScaleAns.Scale90
						? 'bg-byx-red'
						: ''}"
					on:click={() => input.updateQuize5(ScaleAns.Scale90)}
				/>
				<div
					class="border border-black w-10 h-10 rounded-full cursor-pointer {ans ==
					ScaleAns.Scale100
